...
export interface SwaggerTypes<T> {
  request: T;
  swaggerJson: SwaggerJson<T>;
}

export interface SwaggerJson<T = any> {
  swagger: string;
  info: {
    version: string;
    title: string;
...
export type SwaggerPath<T> = Record<string, T>;
...
export interface SwaggerRequestV2 extends SwaggerRequest {
  responses: {
    "200": {
      schema: {
        $ref: string;
...
export interface SwaggerRequestV3 extends SwaggerRequest {
  responses: {
    "200": {
      description: "OK";
      content: {
...
export type SwaggerStringType = TypeCompose<
  { type: "string"; format?: "date-time" },
  SwaggerBaseType
> &
  SwaggerBaseInfo;

export type SwaggerIntegerType = TypeCompose<
  { type: "integer"; format: "int32" | "int64" },
  SwaggerBaseType
> &
  SwaggerBaseInfo;

export type SwaggerBooleanType = TypeCompose<
  { type: "boolean" },
  SwaggerBaseType
> &
  SwaggerBaseInfo;

export type SwaggerNumberType = TypeCompose<
  { type: "number" },
  SwaggerBaseType
> &
  SwaggerBaseInfo;

export type SwaggerQueryRefType = TypeCompose<
  { type: "ref" },
  SwaggerBaseType
> &
  SwaggerBaseInfo;
...
export type SwaggerType = SwaggerBaseType &
  (
    | SwaggerStringType
    | SwaggerRefType
    | SwaggerArrayType
