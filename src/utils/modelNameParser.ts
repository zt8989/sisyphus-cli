...
const logger = createDebugLogger('ModelNameParser')
...
// ES5.1 / Unicode 6.1
const ValidJavaScriptVariableName =
...
export default class ModelNameParser {
  ...
  private _parserGenericString(data: ModelStruct, nameRef: { name: string }) {
    if (Reflect.has(scalarType, data.name)) {
      ...
      nameRef.name += 'any'
    } else if (['List', 'ArrayList', 'Set'].includes(data.name)) {
  ...
  readToken() {
    ...
    if (name[0] === TOKEN.LEFT) {
      ...
      this.position += 1
      return name[0]
    } else if (ValidJavaScriptVariableName.test(name)) {
      const match = name.match(ValidJavaScriptVariableName)
      if (match) {
        this.position += match[0].length
      }
      return match ? match[0] : ''
    } else {
      throw new ParseError(`无法解析: ` + this.name)
    }
  }
